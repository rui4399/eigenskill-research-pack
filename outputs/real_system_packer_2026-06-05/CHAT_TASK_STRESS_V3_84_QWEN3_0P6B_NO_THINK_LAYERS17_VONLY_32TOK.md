...
|---|---:|---:|---:|---:|
| baseline | 46 / 84 | 0.5476 | 21.9215 | 0.103110 |
| fused | 43 / 84 | 0.5119 | 21.1601 | 0.102877 |
...
| fused | `stress_number_000` | `number` | false | `4096` | `<think>  </think>  We are given that there are 8192 signed 4-bit weights. Each byte can hold two 4-bit weights.   ` |
| fused | `stress_number_001` | `number` | false | `65536` | `<think>  </think>  Each row has 4096 signed 4-bit weights, so each row is 4096 bytes. Since there are` |
...
| fused | `stress_number_003` | `number` | false | `21` | `<think>  </think>  The number of layers packed is $ 3 \times 24 = 72 $.   Final answer: **72**` |
...
| fused | `stress_contains_all_002` | `contains_all` | false | `['calibration', 'split', 'instability']` | `<think>  </think>  Calibration robustness is crucial because it ensures that the model's performance remains stable and ` |
| fused | `stress_contains_all_003` | `contains_all` | false | `['compression', 'latency', 'speedup']` | `<think>  </think>  **Caveat: Packed Compression and Real Runtime**  Packed compression is a technique used to reduce the` |
| fused | `stress_contains_all_004` | `contains_all` | false | `['download', 'leakage', 'random']` | `<think>  </think>  A deterministic stress slice should avoid the following:  1. **Unrealistic or unrealistic stress cond` |
| fused | `stress_contains_all_005` | `contains_all` | false | `['device', 'ttft', 'tokens']` | `<think>  </think>  Before claiming a phone deployment, the following must be measured:  1. **Phone Model and Specificati` |
| fused | `stress_contains_none_000` | `contains_none` | true | `['sota']` | `<think>  </think>  A conservative result is one that prioritizes stability, continuity, and long-term sustainability ove` |
| fused | `stress_contains_none_001` | `contains_none` | true | `['official']` | `<think>  </think>  A local diagnostic is a person who checks and tests the health of a person, like a doctor or a nurse,` |
...
| fused | `stress_contains_none_004` | `contains_none` | true | `['delete']` | `<think>  </think>  A C-drive cleanup involves removing or clearing out unnecessary files, folders, or data that are no l` |
| fused | `stress_contains_none_005` | `contains_none` | true | `['universal']` | `<think>  </think>  A benchmark slice is a specific, standardized unit of data or information used to measure performance` |
| fused | `stress_sentence_count_000` | `sentence_count` | true | `{'count': 1}` | `<think>  </think>  The role-policy selector is a component in a system that selects the appropriate role and policy base` |
| fused | `stress_sentence_count_001` | `sentence_count` | false | `{'count': 2}` | `<think>  </think>  Layer 20 expansion can be risky because it increases the number of nodes and potential connections, w` |
| fused | `stress_sentence_count_002` | `sentence_count` | true | `{'count': 1}` | `<think>  </think>  A disk guard is useful in exactly one sentence.` |
| fused | `stress_word_count_000` | `word_count` | false | `{'count': 3}` | `<think>  </think>  Speed.` |
| fused | `stress_word_count_001` | `word_count` | false | `{'count': 4}` | `<think>  </think>  Robust calibration evidence.` |
...
| fused | `stress_all_of_001` | `all_of` | true | `[{'type': 'sentence_count', 'answer': {'count': 1}}, {'type': 'contains_all', 'answer': ['calibration', 'memory']}, {'type': 'contains_none', 'answer': ['universal']}]` | `<think>  </think>  Calibration ensures accurate memory.` |
...
| fused | `stress_all_of_003` | `all_of` | false | `[{'type': 'word_count', 'answer': {'count': 4}}, {'type': 'contains_all', 'answer': ['ttft', 'memory']}]` | `<think>  </think>  1. **Ttft Memory**   2. **Ttft & Memory**   3. **Ttft Memory**   ` |
...
| fused | `stress_mcq_extra_003` | `mcq` | false | `A` | `<think>  </think>  D. Full QKV` |
...
| fused | `stress_number_extra_001` | `number` | true | `20` | `<think>  </think>  The number of tasks failed is $ 42 - 22 = \boxed{20} $.` |
| fused | `stress_number_extra_002` | `number` | false | `49` | `<think>  </think>  4033 rounded down is 4033. To find the percentage, divide by 8151:  $$ ` |
| fused | `stress_number_extra_003` | `number` | true | `1387` | `<think>  </think>  1265 + 122 = 1387  The total number of MB freed is **1387` |
| fused | `stress_number_extra_004` | `number` | true | `0` | `<think>  </think>  0.` |
| fused | `stress_number_extra_005` | `number` | false | `512` | `<think>  </think>  A 4-bit row with 1024 weights uses **1 packed byte**.` |
| fused | `stress_json_keys_extra_000` | `json_keys` | true | `['selected_policy', 'evidence']` | `<think>  </think>  ```json {   "selected_policy": "evidence",   "evidence": "evidence" } ```` |
| fused | `stress_json_keys_extra_001` | `json_keys` | true | `['c_free_gb', 'guard_limit']` | `<think>  </think>  ```json {"c_free_gb": "0", "guard_limit": "1000000"} ```` |
...
| fused | `stress_json_keys_extra_004` | `json_keys` | false | `['benchmark', 'caveat']` | `<think>  </think>  ```json {"benchmark": "caveat"} ```` |
| fused | `stress_json_keys_extra_005` | `json_keys` | false | `['artifact', 'local_only']` | `<think>  </think>  ```json {"artifact": "local_only"} ```` |
| fused | `stress_contains_all_extra_000` | `contains_all` | false | `['q-only', 'larger', 'slice']` | `<think>  </think>  The next validation target for the selected role policy is **[Insert specific validation target, e.g.` |
...
| fused | `stress_contains_all_extra_003` | `contains_all` | false | `['packed', 'runtime', 'evidence']` | `<think>  </think>  A **conservative systems claim** refers to a claim that a certain system or policy is **conservative*` |
| fused | `stress_contains_all_extra_004` | `contains_all` | false | `['memory', 'disk', 'timeout']` | `<think>  </think>  The GPU guard records:   1. **System performance metrics** (CPU, GPU usage, memory usage).   2. **Err` |
| fused | `stress_contains_all_extra_005` | `contains_all` | false | `['split', 'quality', 'regression']` | `<think>  </think>  Role policies need multiple splits for several key reasons:  1. **To Prevent Conflicts Between Roles*` |
| fused | `stress_contains_none_extra_000` | `contains_none` | true | `['final']` | `<think>  </think>  A candidate who only selects Q.` |
| fused | `stress_contains_none_extra_001` | `contains_none` | true | `['proof']` | `<think>  </think>  A stress benchmark is a measure of how much pressure or stress a person experiences under certain con` |
| fused | `stress_contains_none_extra_002` | `contains_none` | true | `['panic']` | `<think>  </think>  C-drive pressure refers to the pressure or strain that a hard drive experiences when it's being used,` |
| fused | `stress_contains_none_extra_003` | `contains_none` | true | `['upload']` | `<think>  </think>  Local artifacts are physical items that are created or used by people in a specific place. They can i` |
| fused | `stress_contains_none_extra_004` | `contains_none` | true | `['bad']` | `<think>  </think>  A selector rejection occurs when the system or algorithm fails to accept a particular option or input` |
...
| fused | `stress_sentence_count_extra_001` | `sentence_count` | false | `{'count': 2}` | `<think>  </think>  Cache migration requires careful planning and execution to prevent data loss, system instability, and` |
...
| fused | `stress_all_of_extra_001` | `all_of` | true | `[{'type': 'sentence_count', 'answer': {'count': 1}}, {'type': 'contains_all', 'answer': ['q-only', 'stress']}, {'type': 'contains_none', 'answer': ['proof']}]` | `<think>  </think>  Q-only and stress are both important for effective communication.` |
