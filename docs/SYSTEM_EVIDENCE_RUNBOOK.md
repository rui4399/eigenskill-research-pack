...
point for the current paper-facing gate set. It fixes the 70 gate paths in one
...
python train_python/build_evidence_ledger.py \
  ...
  --gate official_ptq_task_qwen25_1p5b_mmlu_full_prefix12000_fp16_awq_gptqmodel=outputs/official_ptq_task_qwen25_1p5b_mmlu_full_prefix12000_fp16_awq_gptqmodel_matrix_2026_06_08.json \
  --gate official_ptq_qwen25_1p5b_mmlu_full_prefix12000_fp16_awq_gptqmodel_runtime_profile=outputs/official_ptq_qwen25_1p5b_mmlu_full_prefix12000_fp16_awq_gptqmodel_runtime_profile_2026_06_08.json \
  --gate official_ptq_qwen25_1p5b_mmlu_full_prefix12000_fp16_awq_gptqmodel_statistics=outputs/official_ptq_task_qwen25_1p5b_mmlu_full_prefix12000_fp16_awq_gptqmodel_statistics_2026_06_08.json \
  --gate official_ptq_task_qwen25_1p5b_mmlu_full_prefix14042_fp16_awq_gptqmodel=outputs/official_ptq_task_qwen25_1p5b_mmlu_full_prefix14042_fp16_awq_gptqmodel_matrix_2026_06_08.json \
  --gate official_ptq_qwen25_1p5b_mmlu_full_prefix14042_fp16_awq_gptqmodel_runtime_profile=outputs/official_ptq_qwen25_1p5b_mmlu_full_prefix14042_fp16_awq_gptqmodel_runtime_profile_2026_06_08.json \
  --gate official_ptq_qwen25_1p5b_mmlu_full_prefix14042_fp16_awq_gptqmodel_statistics=outputs/official_ptq_task_qwen25_1p5b_mmlu_full_prefix14042_fp16_awq_gptqmodel_statistics_2026_06_08.json \
...
Current full-MMLU prefix1500 result: the first three 500-row shards are merged
per variant and gated as a 1500-row prefix. FP16 gets 848/1500, AutoAWQ gets
796/1500, and GPTQModel gets 776/1500. The prefix task gate records max
measured drop `0.0480` versus FP16 and peak guard VRAM ratio `0.8730`. The
runtime profile reports FP16 `4.8378` tok/s and `0.415698` s TTFT, AutoAWQ
`10.6969` tok/s and `0.152438` s TTFT, and GPTQModel `8.5655` tok/s and
`0.186657` s TTFT. The paired statistics gate reports AutoAWQ delta `-0.0347`
with CI `[-0.0533, -0.0153]` and GPTQModel delta `-0.0480` with CI
`[-0.0693, -0.0253]`.

Current full-MMLU prefix2000 result: the first four 500-row shards are merged
per variant and gated as a 2000-row prefix. FP16 gets 1128/2000, AutoAWQ gets
1060/2000, and GPTQModel gets 1020/2000. The prefix task gate records max
measured drop `0.0540` versus FP16 and peak guard VRAM ratio `0.8730`. The
runtime profile reports FP16 `4.8733` tok/s and `0.419905` s TTFT, AutoAWQ
`10.8575` tok/s and `0.152379` s TTFT, and GPTQModel `8.7780` tok/s and
`0.189020` s TTFT. The paired statistics gate reports AutoAWQ delta `-0.0340`
with CI `[-0.0500, -0.0170]` and GPTQModel delta `-0.0540` with CI
`[-0.0725, -0.0345]`.

Current full-MMLU prefix2500 result: the first five 500-row shards are merged
per variant and gated as a 2500-row prefix. FP16 gets 1387/2500, AutoAWQ gets
1311/2500, and GPTQModel gets 1252/2500. The prefix task gate records max
measured drop `0.0540` versus FP16 and peak guard VRAM ratio `0.8730`. The
runtime profile reports FP16 `4.9436` tok/s and `0.423534` s TTFT, AutoAWQ
`11.2880` tok/s and `0.154709` s TTFT, and GPTQModel `8.9728` tok/s and
`0.190026` s TTFT. The paired statistics gate reports AutoAWQ delta `-0.0304`
with CI `[-0.0452, -0.0144]` and GPTQModel delta `-0.0540` with CI
`[-0.0712, -0.0368]`.

Current full-MMLU prefix3000 result: the first six 500-row shards are merged
per variant and gated as a 3000-row prefix. FP16 gets 1687/3000, AutoAWQ gets
1601/3000, and GPTQModel gets 1528/3000. The prefix task gate records max
measured drop `0.0530` versus FP16 and peak guard VRAM ratio `0.8730`. The
runtime profile reports FP16 `4.8991` tok/s and `0.420055` s TTFT, AutoAWQ
`11.1693` tok/s and `0.152503` s TTFT, and GPTQModel `8.9680` tok/s and
`0.188292` s TTFT. The paired statistics gate reports AutoAWQ delta `-0.0287`
with CI `[-0.0427, -0.0150]` and GPTQModel delta `-0.0530` with CI
`[-0.0690, -0.0360]`.

Current full-MMLU prefix3500 result: the first seven 500-row shards are merged
per variant and gated as a 3500-row prefix. FP16 gets 1999/3500, AutoAWQ gets
1902/3500, and GPTQModel gets 1824/3500. The prefix task gate records max
measured drop `0.0500` versus FP16 and peak guard VRAM ratio `0.8730`. The
runtime profile reports FP16 `4.8743` tok/s and `0.419125` s TTFT, AutoAWQ
`10.8438` tok/s and `0.160563` s TTFT, and GPTQModel `8.9205` tok/s and
`0.188762` s TTFT. The paired statistics gate reports AutoAWQ delta `-0.0277`
with CI `[-0.0403, -0.0151]` and GPTQModel delta `-0.0500` with CI
`[-0.0646, -0.0346]`.

Current full-MMLU prefix4000 result: the first eight 500-row shards are merged
per variant and gated as a 4000-row prefix. FP16 gets 2366/4000, AutoAWQ gets
2251/4000, and GPTQModel gets 2145/4000. The prefix task gate records max
measured drop `0.0553` versus FP16 and peak guard VRAM ratio `0.8730`. The
runtime profile reports FP16 `4.8154` tok/s and `0.416089` s TTFT, AutoAWQ
`10.7702` tok/s and `0.158048` s TTFT, and GPTQModel `8.8304` tok/s and
`0.187847` s TTFT. The paired statistics gate reports AutoAWQ delta `-0.0288`
with CI `[-0.0405, -0.0173]` and GPTQModel delta `-0.0553` with CI
`[-0.0685, -0.0413]`.

Current full-MMLU prefix4500 result: the first nine 500-row shards are merged
per variant and gated as a 4500-row prefix. FP16 gets 2614/4500, AutoAWQ gets
2495/4500, and GPTQModel gets 2360/4500. The prefix task gate records max
measured drop `0.0564` versus FP16 and peak guard VRAM ratio `0.8730`. The
runtime profile reports FP16 `4.8561` tok/s and `0.417594` s TTFT, AutoAWQ
`10.9480` tok/s and `0.158347` s TTFT, and GPTQModel `8.8469` tok/s and
`0.189934` s TTFT. The paired statistics gate reports AutoAWQ delta `-0.0264`
with CI `[-0.0373, -0.0158]` and GPTQModel delta `-0.0564` with CI
`[-0.0691, -0.0436]`.

Current full-MMLU prefix5000 result: the first ten 500-row shards are merged
per variant and gated as a 5000-row prefix. FP16 gets 2921/5000, AutoAWQ gets
2778/5000, and GPTQModel gets 2647/5000. The prefix task gate records max
measured drop `0.0548` versus FP16 and peak guard VRAM ratio `0.8730`. The
runtime profile reports FP16 `4.8454` tok/s and `0.417038` s TTFT, AutoAWQ
`10.8964` tok/s and `0.157726` s TTFT, and GPTQModel `8.7835` tok/s and
`0.189247` s TTFT. The paired statistics gate reports AutoAWQ delta `-0.0286`
with CI `[-0.0388, -0.0182]` and GPTQModel delta `-0.0548` with CI
`[-0.0670, -0.0426]`.

Current full-MMLU prefix5500 result: the first eleven 500-row shards are merged
per variant and gated as a 5500-row prefix. FP16 gets 3295/5500, AutoAWQ gets
3134/5500, and GPTQModel gets 3000/5500. The prefix task gate records max
measured drop `0.0536` versus FP16 and peak guard VRAM ratio `0.8730`. The
runtime profile reports FP16 `4.8459` tok/s and `0.417699` s TTFT, AutoAWQ
`10.9050` tok/s and `0.157035` s TTFT, and GPTQModel `8.7836` tok/s and
`0.189041` s TTFT. The paired statistics gate reports AutoAWQ delta `-0.0293`
with CI `[-0.0389, -0.0196]` and GPTQModel delta `-0.0536` with CI
`[-0.0649, -0.0418]`.

Current full-MMLU prefix6000 result: the first twelve 500-row shards are merged
per variant and gated as a 6000-row prefix. FP16 gets 3630/6000, AutoAWQ gets
3463/6000, and GPTQModel gets 3320/6000. The prefix task gate records max
measured drop `0.0517` versus FP16 and peak guard VRAM ratio `0.8730`. The
runtime profile reports FP16 `4.8318` tok/s and `0.417520` s TTFT, AutoAWQ
`10.7409` tok/s and `0.161641` s TTFT, and GPTQModel `8.8492` tok/s and
`0.188530` s TTFT. The paired statistics gate reports AutoAWQ delta `-0.0278`
with CI `[-0.0370, -0.0187]` and GPTQModel delta `-0.0517` with CI
`[-0.0627, -0.0407]`.

Current full-MMLU prefix6500 result: the first thirteen 500-row shards are
merged per variant and gated as a 6500-row prefix. FP16 gets 3972/6500, AutoAWQ
gets 3795/6500, and GPTQModel gets 3652/6500. The prefix task gate records max
measured drop `0.0492` versus FP16 and peak guard VRAM ratio `0.8760`. The
runtime profile reports FP16 `4.8411` tok/s and `0.414129` s TTFT, AutoAWQ
`10.7132` tok/s and `0.160706` s TTFT, and GPTQModel `8.8778` tok/s and
`0.187424` s TTFT. The paired statistics gate reports AutoAWQ delta `-0.0272`
with CI `[-0.0362, -0.0183]` and GPTQModel delta `-0.0492` with CI
`[-0.0598, -0.0386]`.

Current full-MMLU prefix7000 result: the first fourteen 500-row shards are
merged per variant and gated as a 7000-row prefix. FP16 gets 4305/7000, AutoAWQ
gets 4121/7000, and GPTQModel gets 3961/7000. The prefix task gate records max
measured drop `0.0491` versus FP16 and peak guard VRAM ratio `0.8794`. The
runtime profile reports FP16 `4.8377` tok/s and `0.411032` s TTFT, AutoAWQ
`10.7648` tok/s and `0.158735` s TTFT, and GPTQModel `8.8689` tok/s and
`0.186104` s TTFT. The paired statistics gate reports AutoAWQ delta `-0.0263`
with CI `[-0.0351, -0.0180]` and GPTQModel delta `-0.0491` with CI
`[-0.0590, -0.0393]`.

Current full-MMLU prefix8000 result: the first sixteen 500-row shards are
merged per variant and gated as an 8000-row prefix. FP16 gets 5059/8000, AutoAWQ
gets 4858/8000, and GPTQModel gets 4665/8000. The prefix task gate records max
measured drop `0.0493` versus FP16 and peak guard VRAM ratio `0.8794`. The
runtime profile reports FP16 `4.8894` tok/s and `0.409530` s TTFT, AutoAWQ
`10.8962` tok/s and `0.157509` s TTFT, and GPTQModel `8.9705` tok/s and
`0.186147` s TTFT. The paired statistics gate reports AutoAWQ delta `-0.0251`
...
Current full-MMLU prefix10000 result: the first twenty 500-row shards are merged
per variant and gated as a 10000-row prefix. FP16 gets 6054/10000, AutoAWQ gets
5814/10000, and GPTQModel gets 5553/10000. The prefix task gate records max
measured drop `0.0501` versus FP16 and peak guard VRAM ratio `0.8794`. The
runtime profile reports FP16 `4.9898` tok/s and `0.406312` s TTFT, AutoAWQ
`11.0766` tok/s and `0.155344` s TTFT, and GPTQModel `9.1207` tok/s and
`0.186622` s TTFT. The paired statistics gate reports AutoAWQ delta `-0.0240`
with CI `[-0.0310, -0.0173]` and GPTQModel delta `-0.0501` with CI
`[-0.0582, -0.0415]`.

Current full-MMLU prefix11000 result: the first twenty-two 500-row shards are
merged per variant and gated as an 11,000-row prefix. FP16 gets 6554/11000,
AutoAWQ gets 6295/11000, and GPTQModel gets 6011/11000. The prefix task gate
records max measured drop `0.0494` versus FP16 and peak guard VRAM ratio
`0.8794`. The runtime profile reports FP16 `5.0045` tok/s and `0.403427` s
TTFT, AutoAWQ `11.0222` tok/s and `0.155157` s TTFT, and GPTQModel `9.1480`
tok/s and `0.185809` s TTFT. The paired statistics gate reports AutoAWQ delta
`-0.0235` with CI `[-0.0301, -0.0168]` and GPTQModel delta `-0.0494` with CI
`[-0.0575, -0.0415]`.

Current full-MMLU prefix12000 result: the first twenty-four 500-row shards are
merged per variant and gated as a 12,000-row prefix. FP16 gets 6968/12000,
AutoAWQ gets 6694/12000, and GPTQModel gets 6389/12000. The prefix task gate
records max measured drop `0.0483` versus FP16 and peak guard VRAM ratio
`0.8794`. The runtime profile reports FP16 `4.9771` tok/s and `0.401457` s
TTFT, AutoAWQ `10.8106` tok/s and `0.157657` s TTFT, and GPTQModel `9.2153`
tok/s and `0.185555` s TTFT. The paired statistics gate reports AutoAWQ delta
