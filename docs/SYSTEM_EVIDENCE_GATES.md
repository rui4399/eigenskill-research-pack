...
Current status: **75 / 75 gates pass**.
...
| `csi_cross_scale_paper_artifacts_qwen25` | calibration robustness | `outputs/CSI_CROSS_SCALE_PAPER_ARTIFACTS_QWEN25_0P5B_VS_1P5B_2026_06_12.md` | Paper-facing same-n and n=2-to-n=8 gain tables plus a dual-curve SVG summarize the 0.5B-vs-1.5B CSI comparison; this is presentation-grade evidence packaging, not new downstream retention. |
...
| `official_gptqmodel_public_calib_1p5b_16_eval` | official PTQ readiness | `outputs/OFFICIAL_GPTQMODEL_PUBLIC_CALIB_QWEN25_1P5B_16_GATE_2026_06_08.md` | A public-calibrated GPTQModel W4 group-128 Qwen2.5-1.5B artifact is freshly quantized under a 90% VRAM guard, saved as an eight-file local artifact, and re-evaluated on 16 WikiText2 plus 16 C4 prompts totaling 2857 PPL tokens with max PPL ratio 1.1363; this is scale-up readiness evidence, not complete GPTQ/AWQ competitiveness or task retention. |
| `official_gptqmodel_task_execution_qwen25_1p5b_subset100` | task execution subset | `outputs/OFFICIAL_GPTQMODEL_TASK_EXECUTION_QWEN25_1P5B_SUBSET100_MATRIX_2026_06_08.md` | The saved Qwen2.5-1.5B GPTQModel W4/G128 artifact reloads through `gptq_torch` and runs matched 100-row MMLU abstract-algebra plus 100-row GSM8K public task subsets under a 90% GPU guard; this is native-package execution evidence, not leaderboard-scale task retention or GPTQ superiority. |
| `official_ptq_task_qwen25_1p5b_subset100_fp16_awq_gptqmodel` | task execution subset | `outputs/OFFICIAL_PTQ_TASK_QWEN25_1P5B_SUBSET100_FP16_AWQ_GPTQMODEL_MATRIX_2026_06_08.md` | FP16, AutoAWQ, and GPTQModel Qwen2.5-1.5B variants run matched 100-row public MMLU/GSM8K task subsets under GPU guard; max drop versus FP16 is 0.08 and peak guard VRAM ratio is 0.8013, but this remains local subset evidence. |
| `official_ptq_qwen25_1p5b_subset100_fp16_awq_gptqmodel_runtime_profile` | runtime profile | `outputs/OFFICIAL_PTQ_QWEN25_1P5B_SUBSET100_FP16_AWQ_GPTQMODEL_RUNTIME_PROFILE_2026_06_08.md` | PC-side TTFT/tokens/s/VRAM are summarized from the 600-task Qwen2.5-1.5B subset100 matrix; AutoAWQ and GPTQModel use less peak guarded VRAM than FP16 but are slower on this local loader path. |
...
| `official_ptq_task_qwen25_1p5b_gsm8kfull_fp16_awq_gptqmodel` | task execution full task | `outputs/OFFICIAL_PTQ_TASK_QWEN25_1P5B_GSM8KFULL_FP16_AWQ_GPTQMODEL_MATRIX_2026_06_08.md` | FP16, AutoAWQ, and GPTQModel Qwen2.5-1.5B variants run all 1319 GSM8K rows under GPU guard; max drop versus FP16 is 0.00834 and peak guard VRAM ratio is 0.8295, but this remains single-task local retention evidence. |
| `official_ptq_qwen25_1p5b_gsm8kfull_fp16_awq_gptqmodel_runtime_profile` | runtime profile | `outputs/OFFICIAL_PTQ_QWEN25_1P5B_GSM8KFULL_FP16_AWQ_GPTQMODEL_RUNTIME_PROFILE_2026_06_08.md` | PC-side TTFT/tokens/s/VRAM are summarized from the 3957-task-execution full-GSM8K matrix; FP16 uses HF CPU/GPU offload, so runtime ratios remain local-profile evidence. |
| `official_ptq_qwen25_1p5b_gsm8kfull_fp16_awq_gptqmodel_statistics` | statistical interval | `outputs/OFFICIAL_PTQ_TASK_QWEN25_1P5B_GSM8KFULL_FP16_AWQ_GPTQMODEL_STATISTICS_2026_06_08.md` | The full-GSM8K matrix has paired bootstrap deltas for AutoAWQ and GPTQModel versus FP16; the minimum bootstrap lower bound is -0.0243, so this reports uncertainty rather than superiority. |
| `official_ptq_task_qwen25_1p5b_mmlu_broad5x20_fp16_awq_gptqmodel` | task execution subset | `outputs/OFFICIAL_PTQ_TASK_QWEN25_1P5B_MMLU_BROAD5X20_FP16_AWQ_GPTQMODEL_MATRIX_2026_06_08.md` | FP16, AutoAWQ, and GPTQModel Qwen2.5-1.5B variants run a five-subject 100-row MMLU fixture under GPU guard; max drop versus FP16 is 0.0600 and peak guard VRAM ratio is 0.8114, but this is not full MMLU. |
| `official_ptq_qwen25_1p5b_mmlu_broad5x20_fp16_awq_gptqmodel_runtime_profile` | runtime profile | `outputs/OFFICIAL_PTQ_QWEN25_1P5B_MMLU_BROAD5X20_FP16_AWQ_GPTQMODEL_RUNTIME_PROFILE_2026_06_08.md` | PC-side TTFT/tokens/s/VRAM are summarized from the 300-task-execution MMLU Broad5x20 matrix; FP16 uses a local guarded HF path, so runtime ratios remain local-profile evidence. |
| `official_ptq_qwen25_1p5b_mmlu_broad5x20_fp16_awq_gptqmodel_statistics` | statistical interval | `outputs/OFFICIAL_PTQ_TASK_QWEN25_1P5B_MMLU_BROAD5X20_FP16_AWQ_GPTQMODEL_STATISTICS_2026_06_08.md` | The MMLU Broad5x20 matrix has paired bootstrap deltas for AutoAWQ and GPTQModel versus FP16; the minimum bootstrap lower bound is -0.1500, so this reports uncertainty rather than superiority. |
| `official_ptq_task_qwen25_1p5b_mmlu_broad10x20_fp16_awq_gptqmodel` | task execution subset | `outputs/OFFICIAL_PTQ_TASK_QWEN25_1P5B_MMLU_BROAD10X20_FP16_AWQ_GPTQMODEL_MATRIX_2026_06_08.md` | FP16, AutoAWQ, and GPTQModel Qwen2.5-1.5B variants run a ten-subject 200-row MMLU fixture under GPU guard; max drop versus FP16 is 0.0350 and peak guard VRAM ratio is 0.8189, but this is not full MMLU. |
| `official_ptq_qwen25_1p5b_mmlu_broad10x20_fp16_awq_gptqmodel_runtime_profile` | runtime profile | `outputs/OFFICIAL_PTQ_QWEN25_1P5B_MMLU_BROAD10X20_FP16_AWQ_GPTQMODEL_RUNTIME_PROFILE_2026_06_08.md` | PC-side TTFT/tokens/s/VRAM are summarized from the 600-task-execution MMLU Broad10x20 matrix; FP16 uses a local guarded HF path, so runtime ratios remain local-profile evidence rather than production speedup. |
| `official_ptq_qwen25_1p5b_mmlu_broad10x20_fp16_awq_gptqmodel_statistics` | statistical interval | `outputs/OFFICIAL_PTQ_TASK_QWEN25_1P5B_MMLU_BROAD10X20_FP16_AWQ_GPTQMODEL_STATISTICS_2026_06_08.md` | The MMLU Broad10x20 matrix has paired bootstrap deltas for AutoAWQ and GPTQModel versus FP16; the minimum bootstrap lower bound is -0.0950, so this reports uncertainty rather than superiority. |
| `official_ptq_task_qwen25_1p5b_mmlu_broad20x20_fp16_awq_gptqmodel` | task execution subset | `outputs/OFFICIAL_PTQ_TASK_QWEN25_1P5B_MMLU_BROAD20X20_FP16_AWQ_GPTQMODEL_MATRIX_2026_06_08.md` | FP16, AutoAWQ, and GPTQModel Qwen2.5-1.5B variants run a twenty-subject 400-row MMLU fixture under GPU guard; max drop versus FP16 is 0.0400 and peak guard VRAM ratio is 0.8201, but this is not full MMLU. |
| `official_ptq_qwen25_1p5b_mmlu_broad20x20_fp16_awq_gptqmodel_runtime_profile` | runtime profile | `outputs/OFFICIAL_PTQ_QWEN25_1P5B_MMLU_BROAD20X20_FP16_AWQ_GPTQMODEL_RUNTIME_PROFILE_2026_06_08.md` | PC-side TTFT/tokens/s/VRAM are summarized from the 1200-task-execution MMLU Broad20x20 matrix; FP16 uses a local guarded HF path, so runtime ratios remain local-profile evidence rather than production speedup. |
| `official_ptq_qwen25_1p5b_mmlu_broad20x20_fp16_awq_gptqmodel_statistics` | statistical interval | `outputs/OFFICIAL_PTQ_TASK_QWEN25_1P5B_MMLU_BROAD20X20_FP16_AWQ_GPTQMODEL_STATISTICS_2026_06_08.md` | The MMLU Broad20x20 matrix has paired bootstrap deltas for AutoAWQ and GPTQModel versus FP16; the minimum bootstrap lower bound is -0.0825, so this reports uncertainty rather than superiority. |
| `official_ptq_task_qwen25_1p5b_mmlu_full_prefix12000_fp16_awq_gptqmodel` | task execution prefix | `outputs/OFFICIAL_PTQ_TASK_QWEN25_1P5B_MMLU_FULL_PREFIX12000_FP16_AWQ_GPTQMODEL_MATRIX_2026_06_08.md` | FP16, AutoAWQ, and GPTQModel Qwen2.5-1.5B variants run the same first 12000 rows of the 57-subject MMLU fixture under GPU guard; max drop versus FP16 is 0.0483 and peak guard VRAM ratio is 0.8794, but this is still prefix evidence rather than complete MMLU. |
| `official_ptq_qwen25_1p5b_mmlu_full_prefix12000_fp16_awq_gptqmodel_runtime_profile` | runtime profile | `outputs/OFFICIAL_PTQ_QWEN25_1P5B_MMLU_FULL_PREFIX12000_FP16_AWQ_GPTQMODEL_RUNTIME_PROFILE_2026_06_08.md` | PC-side TTFT/tokens/s/VRAM are summarized from the 36000-task-execution full-MMLU prefix12000 matrix; AutoAWQ/GPTQModel are faster and use less peak VRAM than the local FP16 path, but this remains guarded PC-side evidence rather than mobile or production deployment. |
| `official_ptq_qwen25_1p5b_mmlu_full_prefix12000_fp16_awq_gptqmodel_statistics` | statistical interval | `outputs/OFFICIAL_PTQ_TASK_QWEN25_1P5B_MMLU_FULL_PREFIX12000_FP16_AWQ_GPTQMODEL_STATISTICS_2026_06_08.md` | The full-MMLU prefix12000 matrix has paired bootstrap deltas for AutoAWQ and GPTQModel versus FP16; the minimum bootstrap lower bound is -0.0558, so this reports uncertainty rather than PTQ superiority. |
| `official_ptq_task_qwen25_1p5b_mmlu_full_prefix14042_fp16_awq_gptqmodel` | task execution full benchmark | `outputs/OFFICIAL_PTQ_TASK_QWEN25_1P5B_MMLU_FULL_PREFIX14042_FP16_AWQ_GPTQMODEL_MATRIX_2026_06_08.md` | FP16, AutoAWQ, and GPTQModel Qwen2.5-1.5B variants run the same complete 57-subject, 14,042-row MMLU fixture under GPU guard; max drop versus FP16 is 0.0502 and peak guard VRAM ratio is 0.8794. |
| `official_ptq_qwen25_1p5b_mmlu_full_prefix14042_fp16_awq_gptqmodel_runtime_profile` | runtime profile | `outputs/OFFICIAL_PTQ_QWEN25_1P5B_MMLU_FULL_PREFIX14042_FP16_AWQ_GPTQMODEL_RUNTIME_PROFILE_2026_06_08.md` | PC-side TTFT/tokens/s/VRAM are summarized from the 42126-task-execution full-MMLU matrix; AutoAWQ/GPTQModel are faster and use less peak VRAM than the local FP16 path, but this remains guarded PC-side evidence rather than mobile or production deployment. |
| `official_ptq_qwen25_1p5b_mmlu_full_prefix14042_fp16_awq_gptqmodel_statistics` | statistical interval | `outputs/OFFICIAL_PTQ_TASK_QWEN25_1P5B_MMLU_FULL_PREFIX14042_FP16_AWQ_GPTQMODEL_STATISTICS_2026_06_08.md` | The complete full-MMLU matrix has Wilson intervals and paired bootstrap deltas for AutoAWQ and GPTQModel versus FP16; the minimum bootstrap lower bound is -0.0570, so this reports uncertainty rather than PTQ superiority. |
| `official_ptq_task_qwen25_1p5b_ifeval_deterministic_v2_fp16_awq_gptqmodel` | task execution smoke | `outputs/OFFICIAL_PTQ_TASK_QWEN25_1P5B_IFEVAL_DETERMINISTIC_V2_FP16_AWQ_GPTQMODEL_MATRIX_2026_06_16.md` | FP16, AutoAWQ, and GPTQModel Qwen2.5-1.5B variants run the same 8-row deterministic IFEval-style fixture under GPU guard; FP16 is 0/8, so this is execution-path evidence rather than instruction-following retention evidence. |
| `official_ptq_qwen25_1p5b_ifeval_deterministic_v2_fp16_awq_gptqmodel_runtime_profile` | runtime profile | `outputs/OFFICIAL_PTQ_QWEN25_1P5B_IFEVAL_DETERMINISTIC_V2_FP16_AWQ_GPTQMODEL_RUNTIME_PROFILE_2026_06_16.md` | PC-side TTFT/tokens/s/VRAM are summarized from the Qwen2.5-1.5B deterministic IFEval-style matrix; this is a local guarded profile, not production or mobile runtime evidence. |
...
| `w4a8_activation_reconstruction_extended` | module reconstruction | `outputs/w4a8_activation_reconstruction_extended_2026_06_08/W4A8_ACTIVATION_RECONSTRUCTION_EXTENDED_GATE.md` | Selected real Qwen3 attention and MLP module activations have measured W4A8 drift bounds, with MLP down projections exposed as the worst integration-risk cases; this is not full-model quality retention or end-to-end speed evidence. |
