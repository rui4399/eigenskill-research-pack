...
| Missing baseline families are explicitly tracked and proxy evidence is separated from faithful official coverage. | Machine-readable baseline coverage manifest and generated gap dashboard now report `proxy_only` and `proxy_without_package_audit` separately from `covered`. | `docs/BASELINE_COVERAGE_MANIFEST.json`, `outputs/BASELINE_GAP_DASHBOARD_2026_06_06.md` | Tracks readiness gaps; AWQ/GPTQ, rotation, and allocation proxies do not count as faithful official baseline completion. |
