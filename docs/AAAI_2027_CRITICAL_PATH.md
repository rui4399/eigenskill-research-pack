...
3. **Task retention.** Move beyond subset100. Full GSM8K is now measured for
   ...
   fixture and guarded FP16/AutoAWQ/GPTQModel shard plan now exist, and offset
   0/limit 500 has matched task, runtime, and statistics gates. The next target
   is executing the remaining 28 shards per variant or extending IFEval under
   the same matched guard protocol.
...
1. Continue the generated full-MMLU plan:
   ...
   GPTQModel variant. The first 500-row shard is gated; full-MMLU completion
