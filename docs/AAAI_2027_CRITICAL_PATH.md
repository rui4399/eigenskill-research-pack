...
|---|---|---|
| Only tiny task subsets. | Reviewers can call the result a toy diagnosis. | Full GSM8K is now covered for both the local 7B public-task row and the guarded Qwen2.5-1.5B FP16/AutoAWQ/GPTQModel row; MMLU has progressed from Broad5x20 to Broad20x20, and a true full-MMLU fixture/plan plus the first matched 9000-row prefix now exist. The remaining 10 full 500-row shards plus the tail per variant and broader IFEval remain open. |
...
3. **Task retention.** Move beyond subset100. Full GSM8K is now measured for
   ...
   first 9000-row prefix has matched task, runtime, and statistics gates. The
...
1. Continue the generated full-MMLU plan:
   ...
   GPTQModel variant. The first 9000-row prefix is gated; full-MMLU completion
