...
| Official PTQ probes | AutoAWQ/GPTQModel Qwen2.5-0.5B W4/G128 readiness, aligned expanded 16-prompt public PPL gates, tiny task smoke, matched 50-row and true 100-row task subset paths, deterministic IFEval-style execution smoke, a matched local baseline pack, a guarded Qwen2.5-1.5B AutoAWQ public-calibration scale-up smoke, and Qwen2.5-1.5B FP16-vs-AutoAWQ matched 20-row plus 100-row MMLU/GSM8K task subset matrices | local evidence only; quantized paths save VRAM but are not production speedups here |
...
Current ledger status: **41 / 41 gates pass**.
...
python train_python/build_current_evidence_ledger.py --expected-gate-count 41
