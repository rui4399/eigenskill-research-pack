...
>
> 状态更新，2026-06-08，2026-06-13 同步：本中文稿保留为中文读者版/历史草稿，不是当前投稿主稿。当前更接近投稿骨架的版本是 `paper_drafts/eigenskill_q_research_draft_en_2026_06_07.md`，它已纳入 75-gate evidence ledger、calibration robustness stress、sensitivity perturbation matrix、calibration seed stability、CSI-vs-n calibration-size curve、CSI trend-significance gate、CSI null-permutation gate、rank-inversion theory gate、consensus transfer boundary、interaction-aware swap boundary、7B public-task GSM8K200/MMLU100 scale row、7B full-GSM8K public-task row、public-task model ladder、official PTQ task-execution smoke、official PTQ matched subset50/subset100 runtime profile、official PTQ deterministic IFEval-style execution/runtime profile、official PTQ matched baseline pack、AutoAWQ/GPTQModel aligned 16-prompt public PPL gates、Qwen2.5-1.5B AutoAWQ/GPTQModel public-calibration scale-up gates、Qwen2.5-1.5B GPTQModel MMLU100/GSM8K100 task-execution subset、Qwen2.5-1.5B FP16/AutoAWQ/GPTQModel matched subset100、GSM8K200/MMLU100、sharded GSM8K500、full GSM8K1319、MMLU Broad5x20/Broad10x20/Broad20x20 和 full-MMLU prefix12000/complete 14042 task/runtime/statistics gates、official PTQ readiness matrix 和 paper-evidence alignment gate。若两者不一致，以新版英文稿和 `docs/PAPER_CLAIM_MATRIX.md` 为准。

## 摘要

大语言模型在资源受限环境中的部署受到显存、访存带宽和推理路径共同限制。后训练量化是降低成本的重要路径，但统一低比特量化往往在关键模块引入过大误差，而混合精度分配又依赖少量校准样本估计模块敏感度，容易受到校准分割噪声影响。本文将问题收敛为：在很小校准预算下，模块敏感度排序到底有多不稳定，以及跨分割一致性是否能降低错误位宽分配的风险。EigenSkill-Q 当前是一份可复现实验工件，而不是生产级量化器。它分别在 WikiText2/C4 等校准视图上估计线性模块的低比特损失增量，在固定平均位宽预算下构建 consensus allocation，并通过 claim matrix 与 evidence ledger 约束每一条可公开声明。当前英文主稿对应 70 个通过的 evidence gates，覆盖校准不稳定性、prompt-seed 稳定性、CSI-vs-n、CSI trend significance、CSI null permutation 与 rank-inversion theory gate、随机基线压力、跨分割迁移、交互式 swap 边界、Q-Palette 风格拉格朗日分配代理、7B public-task GSM8K200/MMLU100 scale row、7B full-GSM8K public-task row、AutoAWQ/GPTQModel Qwen2.5-0.5B W4/G128 对齐 16-prompt public PPL readiness、Qwen2.5-1.5B AutoAWQ/GPTQModel public-calibration scale-up readiness、Qwen2.5-1.5B GPTQModel MMLU100/GSM8K100 task-execution subset、Qwen2.5-1.5B FP16/AutoAWQ/GPTQModel matched subset100、GSM8K200/MMLU100、sharded GSM8K500、full GSM8K1319、MMLU Broad5x20/Broad10x20/Broad20x20 与 full-MMLU prefix12000/complete 14042 task/runtime/statistics gates，以及 PC 侧 subset50/subset100 runtime/VRAM 负结果。实验表明，跨数据集一致性能够把“有用但不稳定”的敏感度信号转化为更稳健的短切片 fake-quant 分配依据；同时，官方 PTQ readiness 结果只说明本地基线包可执行，并不支持 SOTA、生产 runtime、移动端部署或端到端加速声明。
