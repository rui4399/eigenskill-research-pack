...
def read_esmp(path: Path) -> EsmpMatrix:
    ...
    for row in range(int(rows)):
        ...
        row_bits[row] = data[offset]
        row_scales[row] = struct.unpack_from("<f", data, offset + 4)[0]
        row_offsets[row] = struct.unpack_from("<Q", data, offset + 8)[0]
...
def _decode_int4_row(row_bytes: np.ndarray, cols: int) -> np.ndarray:
...
def _decode_generic_row(payload: bytes, bit_offset: int, bits: int, cols: int) -> np.ndarray:
...
def dequantize_esmp_weight(esmp: EsmpMatrix) -> torch.Tensor:
