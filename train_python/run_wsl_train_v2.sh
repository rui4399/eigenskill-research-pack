...
python3 train_python/generate_skill_data_v2.py \
...
python3 train_python/eval_skills.py \
  --model HuggingFaceTB/SmolLM2-360M-Instruct \
  --adapter models/eigenskill-smollm2-360m-lora-v2-fp16 \
  --data data_eval/eigenskill_v2/eval.jsonl \
  --out outputs/eigenskill_v2_eval.json \
  --max-new-tokens 120

CUDA_VISIBLE_DEVICES="" python3 train_python/export_model.py \
  --model HuggingFaceTB/SmolLM2-360M-Instruct \
  --adapter models/eigenskill-smollm2-360m-lora-v2-fp16 \
  --out models/eigenskill-smollm2-360m-merged-v2-fp16 \
  --precision fp16

CUDA_VISIBLE_DEVICES="" python3 train_python/export_int8_dynamic.py \
  --model-dir models/eigenskill-smollm2-360m-merged-v2-fp16 \
  --out models/eigenskill-smollm2-360m-int8-v2-dynamic
